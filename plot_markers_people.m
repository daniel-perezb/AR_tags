function plot_markers_people(Markers , master, people)
...
for i = 1:length(people)
    % Location of markers
    x = people(i).location(1);
    y = people(i).location(2);
    z = people(i).location(3);

    % Plot marker center
    text(x+.2, y+.2, z+.2, num2str(i), 'FontSize', 12, 'HorizontalAlignment', 'center','Color','b'); % Print number at the marker position

    % Draw orientation axes
    quiver3(x, y, z, ...
        people(i).orientation(1, 1)*axisLength_people, people(i).orientation(1, 2)*axisLength_people, people(i).orientation(1, 3)*axisLength_people, ...
        'r', 'LineWidth', 2); % Red for the x axis
    quiver3(x, y, z, ...
        people(i).orientation(2, 1)*axisLength_people, people(i).orientation(2, 2)*axisLength_people, people(i).orientation(2, 3)*axisLength_people, ...
        'g', 'LineWidth', 2); % Green for the y axis
    quiver3(x, y, z, ...
        people(i).orientation(3, 1)*axisLength_people, people(i).orientation(3, 2)*axisLength_people, people(i).orientation(3, 3)*axisLength_people, ...
        'b', 'LineWidth', 2); % Blue for the z axis
end
